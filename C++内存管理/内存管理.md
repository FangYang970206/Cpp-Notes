...
还剩下array new/array delete和replacement new，现在来讲array new和array delete。
...
namespace jj02
{
    ...
    void test_call_ctor_directly()
    {
        ...
        for(int i = 0; i < size; ++i)
            new (tmp++) A(i);  		//replacement new, 3次 ctor
...
上面的程序出现了replacement new，用法很奇特，`new (tmp++) A(i)`, new带了参数tmp, 这句话的意思是在tmp指针所指向的对象A中里面调用它的构造函数。

接下来进入replacement new和replacement delete。
